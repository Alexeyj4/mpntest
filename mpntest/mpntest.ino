...
bool bad=true; //bad flag MPN is bad
bool load_fail=false; //load fail flag flag
bool dbg=false; //dbg mode flag (for developers)

//7.5v v6_min_LV_open*7.5/6.32
const int U_overvoltage =616;
...
//*********test 1*******
  ...
  digitalWrite(LV_on, HIGH); //set 10v on
  ...
  if (dbg==true) delay(4000);//debug
  
  if(digitalRead(load_sens)==0) load_fail=true;
...
//*********test 2*******
  ...
  if(digitalRead(load_sens)==1) load_fail=true;
...
//*********test 3*******
  ...
  if((Vmin < analogRead(Uout_meas)) && (analogRead(Uout_meas) < Vmax)) lcd.print("48-OK"); else {lcd.print("48L-BAD"); bad=true;};   
...
//*********test 4*******
  ...
  if((Vmin < analogRead(Uout_meas)) && (analogRead(Uout_meas) < Vmax)) lcd.print("48L-OK"); else {lcd.print("48L-BAD"); bad=true;};  
  ...
  if (bad==false and load_fail==true)  //load fail
  {
    digitalWrite(red_led, HIGH);
    digitalWrite(green_led, LOW);
    message("LOAD FAIL !!!");
...
{
  delay(5);
  if(analogRead(Uout_meas) > U_overvoltage)
  {
    digitalWrite(LV_on, LOW); //set 10v off
    digitalWrite(HV_on, LOW); //set 48v on  

    digitalWrite(red_led, HIGH); // "NOT OK" led
    digitalWrite(green_led, LOW);
